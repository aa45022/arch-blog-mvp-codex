...
export default function PostForm({ postId }: PostFormProps) {
  ...
  if (loading) {
    return <div className="max-w-3xl mx-auto px-4 py-8"><p className="text-sm text-gray-400">載入中...</p></div>;
  }

  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <h1 className="text-lg font-bold text-gray-900 mb-6">{isEdit ? "編輯文章" : "新增文章"}</h1>

      <form onSubmit={handleSubmit} className="space-y-5">
        {/* 標題 */}
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1.5">標題 <span className="text-red-400">*</span></label>
        ...
        {/* Slug */}
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1.5">網址代稱 <span className="text-red-400">*</span></label>
          <div className="flex items-center gap-1 text-xs text-gray-400 mb-1">
            <span>/posts/</span><span className="text-accent">{slug || "..."}</span>
          </div>
        ...
        {/* 摘要 */}
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1.5">摘要 <span className="text-red-400">*</span></label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-xs font-medium text-gray-500">分類 <span className="text-red-400">*</span></label>
              ...
              <div className="space-y-1">
                <div className="flex gap-1">
                  ...
                  <button type="button" onClick={handleAddCategory} disabled={addingCategory}
                    className="bg-accent text-white text-xs px-3 py-2 rounded-lg hover:bg-accent-dark disabled:opacity-50">
                    {addingCategory ? "..." : "新增"}
                  </button>
                </div>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1.5">發佈狀態</label>
            <label className="flex items-center gap-2 mt-2 cursor-pointer">
              <input type="checkbox" checked={published} onChange={(e) => setPublished(e.target.checked)}
                className="accent-accent" />
              <span className="text-sm text-gray-700">{published ? "已發佈" : "草稿"}</span>
            </label>
          </div>
        </div>
        ...
        {/* 標籤 */}
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1.5">標籤</label>
        ...
        {/* 封面圖 */}
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1.5">封面圖（選填）</label>
          <ImageUploader value={coverImage} onChange={setCoverImage} />
        </div>

        {/* 內容 */}
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1.5">內容 <span className="text-red-400">*</span></label>
